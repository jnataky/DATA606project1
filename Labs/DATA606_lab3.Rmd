...
and the streak of length 0 means one miss occurred after a first miss that
ended a streak.
...
```{r data_frame-streak-kobe}
# Convert numerical vector to data frame with col.names = length

kobe_streak <- as.data.frame(kobe_streak)
names(kobe_streak)[1] <- "length"
kobe_streak
```
...
From the above plot, we can see that the distribution of Kobe's streak lengths
from the 2009 NBA finals is unimodal and right-skewed. The typical length of a
streak is 0, and the longest streak of baskets is of length 4.
...
In my simulation of flipping the unfair coin 100 times,
...
```{r data-frame-sim-streak}
# Convert numerical vector to data frame with col.names = length

sim_streak <- as.data.frame(sim_streak)
names(sim_streak)[1] <- "sim_length"

```


```{r plot-sim-streak}
ggplot(data = sim_streak, aes(x = sim_length)) +
  geom_bar()
```


The distribution of simulated independent shooter with a 45% shooting percentage
is unimodal and right-skewed. The typical length of a
streak is 0, and the longest streak of baskets is of length 4 (and 6)
...
With the conditions set (prop = c(0.45, 0.55)), 
the same possible outcomes(H or M), 
and total outcomes (133), I wouldn't expect much of the difference between 
different simulations.
Thus, If I was to run the simulation of the independent shooter a second time,
I would expect its streak distribution to compare to the distribution from the 
question above to be somewhat similar.
...
Kobe Bryant's distribution of streak lengths and the distribution
of streak lengths for the simulated shooter look very similar.
It is an independent shooter with same same shooting percentage as 
Kobe Bryant, so there is no evidence that the hot hand.
model fits Kobe's shooting patterns.
