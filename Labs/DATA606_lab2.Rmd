...
Each of histograms has different number of bins and groups data depending
on the binwidth which can show some details particular a given histogram. 
Therefore, there are features revealed in one that are obscured in another. 
For example the departure delay peaks are not seen in histogram 
...
The distribution is unimordal and right-skewed with some extreme values 
to the right (outliers).

Based on the shape described above, I will then use median and interquartile 
on my summary.

```{r sfo-feb-arr-delay-summary}
...
Calculate the median and interquartile range for `arr_delay`s of flights in
...
Delta and United airlines have the most variable delays.
...
July is the month with the highest average departure delay from a NYC airport.
...
The month of July has the highest average departure delay and December has
the highest median departure delay.
The mean is the more reliable measure for deciding which month(s) to avoid 
flying if you really dislike delayed flights since it gives us the true average.
That is even the pros of choosing the mean and the cons is just the measure is
sensitive to extreme delays. The cons of choosing the median is that the 
measure doesn't arise the true delay although its pros is that it's not 
sensitive to extreme delays as the mean.
...
Based on on time departure percentage, I would choose to fly out of LGA.
...
From the plot above we can see that as the distance increases, the avg_speed 
increases as well. Thus, there is a positive association between 
...
zoom a bit the plot above (reduce the dep_delay & arr_delay interval/axis)

```{r plot-to-replicate-zoom}
ggplot(data = dl_aa_ua, aes(x = dep_delay, y = arr_delay, color = carrier)) +
  xlim (-20,60) +
  ylim (-50,200) +
  geom_point()
...
Roughly, the cutoff point for departure delays where we can still expect 
to get to our destination on time is between 20 and somewhere around 35 minutes.
