...
(a) Avg birth weight = 123.05 - 8.94*(smoking status)

(b) For a smoking status of a mother,
we would expect the birth weight of babies (ounces) to decrease by 8.94

```{r}

birth_weight_of_babies_born_to_smoker <- 123.05 - 8.94*1
c("The predicted birth weight of babies born to smoker is", birth_weight_of_babies_born_to_smoker)

birth_weight_of_babies_born_to_non_smoker <- 123.05 - 8.94*0
c("The predicted birth weight of babies born to non-smoker is", birth_weight_of_babies_born_to_non_smoker)

```

(c) From the table above, p-values are equal to 0 (p_value < 0.05),
we can say that there is a statistically significant relationship between
...
(a) days_absent = 18.93 - 9.11(eth) + 3.10(sex) + 2.15(lrn)

(b) 

eth: For not aboriginal ethnic background students,
we would expect the days absent to decrease by 9.11

sex: For male students,
we would expect the days absent to increase by 3.10

lrn: For slow learner students,
we would expect the days absent to increase by 2.15
...
```{r}
R_square <- 1 - (240.57/264.17)
round(R_square, 3)

```

```{r}
Adjusted_R_square <- 1 - ((240.57/(146-3-1))/(264.17/(146-1)))
round(Adjusted_R_square,3)
...
"No learner status" has the highest adjusted r_square,
thus it should be removed from the model first.
...
The lowest temperatures have more number of damaged O-rings.

We can say somehow that as the temperature increases,
the number of damaged O-rings decreases.

(b)
The slop:
For a damaged O-ring,
we would expect the temperature (F) to decrease by 0.2162

P_value:
p_value < 0.05,
we can say that there is a statistically significant relationship between
the temperature and the number of O-rings.  

(c)

\begin{align*}
\log\left( \frac{{p}}{1 - {p}} \right) = 11.6630 - 0.2162\times Temperature
\end{align*}

(d)

Based on the model, we can see that p-values are very small and closer to 0
(p_value < 0.05), I would say that concerns regarding O-rings are justified.
...
p_51 <- round(exp(11.6630 - 0.2162 * 51) / (1 + exp(11.6630 - 0.2162 * 51)), 3)
p_51


p_53 <- round(exp(11.6630 - 0.2162 * 53) / (1 + exp(11.6630 - 0.2162 * 53)), 3)
p_53

p_55 <- round(exp(11.6630 - 0.2162 * 55) / (1 + exp(11.6630 - 0.2162 * 55)), 3)
p_55
...
# Create a data frame (table for temp and prob)

df <- data.frame(temp, prob)

# Plot

plot(df, type = "b", xlab = 'Temperature(F)', ylab ="Probability", main = "Model-estimated prob curve")
...
The concern that we have applying logistic regression is the sample size 
which is very small (23 shuttle missions) since the requires assumptions are:

- Each outcome is independent of other outcomes:  This can be proven
- Each predictor is linearly related to logit prob if all other are constant: 
sample size is small making this hard to be proven.
